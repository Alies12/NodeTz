...
export class Triangle extends BaseFigure implements ITriangle {
    ...
    constructor(a: number, b: number, c: number, id?: FigureId) {
        ...
        if (a + b <= c || a + c <= b || b + c <= a) {
            throw new GeometryError('The given sides do not satisfy the triangle inequality.');
        }
        this._sideA = a;
        this._sideB = b;
        this._sideC = c;
     ...
     * @private
     */
    private _validateAndSet(sideKey: '_sideA' | '_sideB' | '_sideC', newValue: number): void {
        if (newValue <= 0) {
        ...
        if (sides.a + sides.b <= sides.c || sides.a + sides.c <= sides.b || sides.b + sides.c <= sides.a) {
            throw new GeometryError('Changing this side violates the triangle inequality.');
        }
